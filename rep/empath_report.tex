...
trapezoid EMG channels, and one zygomaticus EMG channel. Video was captured at 25~fps.
...
include mean activation, standard deviation, and an asymmetry index computed as the signed
difference between trapezoid and corrugator channels. Cross-modal interaction features
(GSR $\times$ ECG correlation, GSR $\times$ ECG mutual information) were included to capture
autonomic co-activation patterns not encoded in univariate statistics.

\begin{table*}[t]
\centering
\caption{Complete Biosignal Feature Set (35 Features)}
\label{tab:features}
\small
...
\multirow{7}{*}{GSR}
  ...
  & \texttt{gsr\_sim\_corr}  & GSR $\times$ ECG Pearson correlation \\
  & \texttt{gsr\_sim\_mi}    & GSR $\times$ ECG mutual information \\
  & \texttt{gsr\_x\_ecg}     & Element-wise product mean (co-activation) \\
\midrule
\multirow{6}{*}{ECG}
  ...
  & \texttt{ecg\_sim\_corr}  & ECG self-correlation \\
  & \texttt{ecg\_sim\_mi}    & ECG mutual information \\
\midrule
\multirow{4}{*}{HRV}
  & \texttt{hrv\_meannn}     & Mean RR interval (ms) \\
  & \texttt{hrv\_sdnn}       & RR interval standard deviation \\
  & \texttt{hrv\_rmssd}      & Root mean square successive differences \\
  & \texttt{hrv\_pnn50}      & Proportion of RR diffs $>$ 50 ms \\
\midrule
\multirow{18}{*}{EMG}
  & \texttt{emg\_trap\_mean}       & Trapezoid channel mean activation \\
  & \texttt{emg\_trap\_std}        & Trapezoid channel standard deviation \\
  & \texttt{emg\_trap\_max}        & Trapezoid channel peak activation \\
  & \texttt{emg\_trap\_min}        & Trapezoid channel minimum \\
  & \texttt{emg\_trap\_slope}      & Trapezoid linear trend (polyfit) \\
  & \texttt{emg\_trap\_shannon}    & Trapezoid Shannon entropy \\
  ...
  & \texttt{emg\_corr\_min}        & Corrugator channel minimum \\
  & \texttt{emg\_corr\_slope}      & Corrugator linear trend (polyfit) \\
  ...
  & \texttt{emg\_zyg\_max}         & Zygomaticus channel peak activation \\
  & \texttt{emg\_zyg\_min}         & Zygomaticus channel minimum \\
  & \texttt{emg\_asymmetry}        & Trapezoid $-$ Corrugator mean \\
  & \texttt{emg\_zyg\_corr\_ratio} & Zygomaticus / Corrugator ratio \\
\bottomrule
\end{tabular}
\end{table*}
...
yielding a temporal resolution of approximately one frame per 230 milliseconds. This
sampling rate was chosen to capture low-frequency facial deformation dynamics (brow
lowering, mouth opening) while avoiding redundant computation. MediaPipe FaceMesh
...
model training, ensuring strict evaluation of cross-subject generalization.
...
interpretability, computing mean absolute SHAP values across all 2,680 samples.
...
Of 1,340 PA2 trials, 875 were correctly classified (65.3\%) and 461 were misclassified
...
\addplot coordinates {
  (25,1)(35,1)(45,5)(55,21)(65,16)(75,9)(85,4)(95,0)
};
\end{axis}
\end{tikzpicture}
...
(\textit{SD} = 14.1\%). Four subjects achieve $\geq$90\% accuracy; three subjects achieve
$\leq$43\%.}
\label{fig:persubj}
\end{figure}
...
Notably, 16.4\% of subjects show performance below 55\%, indicating that the model
essentially fails for these individuals. At the other extreme, 14.9\% achieve $\geq$85\%
...
Implementation details for all variants are available in
\texttt{SRC/preprocessing/}.
...
analysis, showing that 35.8\% of subjects achieve $\geq$70\% accuracy, suggests that a
...
However, the 16.4\% of subjects for whom the model performs below 55\% represents a
...
the manuscript. Dr.~Jianchen Shan provided supervision and feedback.
